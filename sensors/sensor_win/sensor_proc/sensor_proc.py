...
"""
Sensor that wraps the system proc command.
"""


async def assess_proc(message_stub, config, message_queue):
    """
    ...
    print(" ::starting proc check-summing")
    print("    $ repeat-interval = %d" % (repeat_delay,))

    ps_canonical_path = which_file("proc")
...
async def proc(message_stub, config, message_queue):
    """
    Run the proc command and push messages to the output queue.
    ...
    p = subprocess.Popen(["proc", "-r", "%d" % (repeat_delay,)], stdout=subprocess.PIPE)
    async for line in p.stdout:

        # slice out the TYPE of the proc message
        ...
        log_level = "debug"
        if line_type in proc_type_map:
            log_level = proc_type_map[line_type]
...
if __name__ == "__main__":
    ...
    wrapper = SensorWrapper("proc", [proc, assess_proc])
    wrapper.start()
