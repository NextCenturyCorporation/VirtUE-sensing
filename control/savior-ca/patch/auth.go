...
func (p Standard) Token(req []byte) (token []byte, err error) {
    log.Warningf("Server key is: %s\n", hex.EncodeToString(p.key))
    log.Warningf("Server additional data is: %s\n", p.ad)
    log.Warningf("Server is using request value [%s]", req)
...
func (p Standard) Verify(ad *AuthenticatedRequest) bool {
	...
	log.Warningf("Server calculated token in hex is: %s\n", hex.EncodeToString(token))
	log.Warningf("Server calculated token raw is: %s\n", token)
	log.Warningf("Client token in hex is %s\n", hex.EncodeToString(ad.Token))
	log.Warningf("Client token raw is %s\n", ad.Token)
